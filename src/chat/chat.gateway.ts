...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: AuthenticatedSocket) {
    ...
    if (existingServerId) {
      // duplicate connect 이벤트 publish
      await this.publishDuplicateConnect(existingServerId, broadcaster.user_id, registerId);
    }
   ...
   */
  private async publishDuplicateConnect(existingServerId: string, broadcasterId: string, registerId: string) {
    ...
    if (this.chatRooms.has(broadcasterId)) {
      ...
      if (roomMap.has(userId)) {
        ...
        if (existingClient) {
          ...
          // deleteChatRoomUser를 사용하여 완전한 정리 작업 수행
          await this.deleteChatRoomUser(broadcasterId, userId);
  ...
  async deleteChatRoomUser(broadcasterId: string, userId: string) {
    ...
    if (this.chatRooms.has(broadcasterId)) {
      ...
      if (roomMap.has(userId)) {
        ...
        console.log(`redis에서 연결 정보 삭제: con:${broadcasterId}, hash key: ${userId}`);
        // Redis에서 연결 정보 삭제
